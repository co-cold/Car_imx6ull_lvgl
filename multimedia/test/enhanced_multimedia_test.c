...
#define LONG_TEST_DURATION 60 // 长时间测试时长（秒）
...
void signal_handler(int sig) {
    printf("\n[INFO] Received signal %d, stopping tests gracefully...\n", sig);
    g_test_running = 0;
}
...
int test_custom_media_like_play(PlayerCore *pc) {
    ...
    if (player_core_play_audio(pc, TEST_AUDIO_FILE) != 0) {
        LOGE("Failed to switch to audio\n");
        return -1;
    }
    sleep(TEST_DURATION);
...
int main(int argc, char *argv[]) {
    ...
    // 输出测试结果
    printf("\n=========================================\n");
    printf("Enhanced Test Results:\n");
    printf("Total Tests: %d\n", total_tests);
    printf("Passed: %d\n", total_tests - failed_tests);
    printf("Failed: %d\n", failed_tests);
    printf("Success Rate: %.2f%%\n", total_tests > 0 ? (float)(total_tests - failed_tests) * 100.0 / total_tests : 0.0);
    printf("=========================================\n");
    
    if (failed_tests > 0) {
        printf("[RESULT] Some tests FAILED - potential crashes detected!\n");
        return -1;
    } else {
        printf("[RESULT] All tests PASSED - no crashes detected in enhanced test suite\n");
        return 0;
    }
}